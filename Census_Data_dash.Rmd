...
Visualizing the median household income (in \$) by race

## Row

### Household income by race for Black, White, and Asian in CA, FL, PA
...
med_inc_race %>%
  ...
  filter(NAME %in% c("California", "Florida", "Pennsylvania")) %>%
...
ca_county %>% 
  mutate(NAME = gsub(" County, California", "", NAME)) %>%
  mutate(percent_black = 100 * (black_popE / summary_est)) %>%
  select(NAME, median_household_incomeE, percent_black) %>%
  rename(state = NAME, "median household income" = median_household_incomeE) %>%
...
renderPlot({
  hist(ca_wrangled$"median household income", probability = TRUE, breaks = as.numeric(input$n_breaks),
       xlab = "Duration (minutes)", main = "Geyser Eruption Duration")
  
  dens <- density(ca_wrangled$percent_black, adjust = input$bw_adjust)
  lines(dens, col = "blue")
})
```
